...
# Helpers
def _conv3x3x3(channels, stride, in_channels):
    return nn.Conv3D(channels, kernel_size=3, strides=stride, padding=1,
...
class BasicBlockV1(HybridBlock):
    ...
    def __init__(self, channels, stride, downsample=False, in_channels=0, **kwargs):
        ...
        self.body.add(_conv3x3x3(channels, stride, in_channels))
        self.body.add(nn.BatchNorm())
        self.body.add(nn.Activation('relu'))
        self.body.add(_conv3x3x3(channels, 1, channels))
        self.body.add(nn.BatchNorm())
        if downsample:
            self.downsample = nn.HybridSequential(prefix='')
            self.downsample.add(nn.Conv3D(channels, kernel_size=1, strides=stride,
                                          use_bias=False, in_channels=in_channels))
...
class BottleneckV1(HybridBlock):
    ...
    def __init__(self, channels, stride, downsample=False, in_channels=0, **kwargs):
        ...
        self.body = nn.HybridSequential(prefix='')
        self.body.add(nn.Conv3D(channels//4, kernel_size=1, strides=1))
        self.body.add(nn.BatchNorm())
        self.body.add(nn.Activation('relu'))
        self.body.add(_conv3x3x3(channels//4, stride, channels//4))
        self.body.add(nn.BatchNorm())
        self.body.add(nn.Activation('relu'))
...
# Specification
resnet_spec = {18: ('basic_block', [2,2, 2, 2], [32, 32, 64, 128, 256]),               ##CH 64-->32,128-->64
